...
@dataclass(frozen=True)
class RelevanceValidationSummary:
    total_articles: int
    relevant: int
    irrelevant: int
...
def validate_relevance_csv(input_path: Path) -> RelevanceValidationSummary:
    """Validate independent human relevance labels before AI-assisted labelling."""
    with input_path.open(encoding="utf-8-sig", newline="") as input_file:
        reader = csv.DictReader(input_file)
        if tuple(reader.fieldnames or ()) != LABEL_FIELDS:
            raise LabelValidationError(
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        for asset in CryptoAsset:
            prefix = asset.value.lower()
            if row[f"{prefix}_sentiment"].strip() or row[
                f"{prefix}_category"
            ].strip():
                errors.append(
                    f"row {row_number}: asset labels must remain blank in the "
                    "human relevance file"
                )
                break
    ...
    return RelevanceValidationSummary(
        total_articles=len(rows),
        relevant=relevant_count,
        irrelevant=irrelevant_count,
    )
